...
def get_browser_headers():
    """Generates authentic browser headers to bypass strict WAF/Cloudflare security blocks."""
    return {
...
def process_website_pipeline_direct(url):
    ...
    try:
        # Inject full browser footprint identity
        session.headers.update(get_browser_headers())
        response = session.get(url, timeout=10)
        if response.status_code != 200:
            print(f"Failed to access base URL {url} - Status Code: {response.status_code}")
            return
        ...
        for sub_url in links_to_crawl:
            try:
                time.sleep(random.uniform(0.6, 1.5)) # Politeness delay to prevent automated rate blocks
